...
mainf.init = (argv) => {
	...
	if (argv2 == 'r' || argv2 == 'release') {
		mainf.checkConfigJson(function(err, json) {
			...
			if (argv.indexOf('-m') != -1) {
				mIndex = argv.indexOf('-m') + 1;
				commit_msg = argv.slice(argv.indexOf('-m') + 1).join('') || 'update';
			}
			if (mIndex && mIndex <= argv.length) {
				arrFiles = argv.slice(3, mIndex - 1);
			} else {
				arrFiles = argv.slice(3);
			}
			mainf.releaesFiles(dir, arrFiles, commit_msg);
			// console.log(dir, arrFiles, commit_msg);
		});
	}
};
...
mainf.help = () => {
	...
	content = content.concat([
		...
		'',
		'    r, release 		把开发环境的文件/文件夹移动到生产环境的文件目录并且svn提交	e.g.: r 123/ app.js -m update',
		'      -m 				svn提交的注释',
		'',
 ...
 * @return {undefined}
 */
mainf.releaesFiles = (dir, files, commit_msg) => {
	if (!dir || !util.isArray(files)) {
		return log.error('参数不正确');
	}
